...
ddrm_old.py
-----------
Restores a degraded RF image with DDRM and saves the result in [-1, 1] range,
without interpolation or additional normalization of the degraded input.
...
def get_args():
    p = argparse.ArgumentParser(description="DDRM restore degraded RF without interpolation")
...
def main():
    ...
    # 1) Load degraded RF (exact size, e.g., 256x256)
    rf_deg = load_arr(args.degraded_mat, "rf_degraded")
    H, W = rf_deg.shape
    rf_t = torch.tensor(rf_deg, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # (1,1,H,W)
    x0 = rf_t.repeat(1, 3, 1, 1).to(device)  # (1,3,H,W)

    # 2) Build degradation operator
    H_funcs = build_H(args.psf_path, H, device)
